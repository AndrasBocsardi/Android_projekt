...
class ActivitiesAdapter(
    ...
    inner class DataViewHolder(itemView: View) : SimpleDataViewHolder(itemView),
        View.OnClickListener, View.OnLongClickListener {
        val activityFromText: TextView = itemView.findViewById(R.id.activity_from_text_view)
        val activityToText: TextView = itemView.findViewById(R.id.activity_to_text_view)
        val activityCreateTime: TextView = itemView.findViewById(R.id.editTextDate)
        ...
        return when (viewType) {
            TaskListItemType.SIMPLE.value -> {
                val itemView = LayoutInflater.from(parent.context)
                    .inflate(R.layout.fragment_activity_list_item, parent, false)
...
//        if (getItemViewType(position) == TaskListItemType.COMPLEX.value) {
        ...
        complexHolder.activityFromText.text = "From: " + usersViewModel.getUserNameByID(currentItem.createdBy)
        complexHolder.activityToText.text =" To: " + usersViewModel.getUserNameByID(currentItem.subUserId)
        complexHolder.activityCreateTime.text = App.epochToDateString(currentItem.createdTime/1000)
  }
